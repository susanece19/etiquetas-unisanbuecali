...
def parse_chemical_db(content_str, sga_files, epp_files, un_files):
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        # Si la fila coincide exactamente con el número de encabezados
        if num_cols == len(headers) and len(hdr_map) >= 8:
            p_name = clean_row[hdr_map.get("nombre", 0)]
            c_text = clean_row[hdr_map.get("composicion", 1)].replace("|", "\n")
            signal_raw = clean_row[hdr_map.get("palabra", 2)]
            sga_raw = clean_row[hdr_map.get("sga", 3)]
            h_raw = clean_row[hdr_map.get("frase_h", 4)]
            p_raw = clean_row[hdr_map.get("frase_p", 5)]
            un_class_raw = clean_row[hdr_map.get("clase_un", 6)]
            un_code_raw = clean_row[hdr_map.get("codigo_un", 7)]
            epp_raw = clean_row[hdr_map.get("epp", 8)]
            prov_text = clean_row[hdr_map.get("proveedor", 9)] if hdr_map.get("proveedor", 9) < num_cols else ""
        else:
            # Algoritmo de extracción por anclas y patrones cuando hay delimitadores no escapados dentro de los campos
            p_name = clean_row[0]
            
            # Anclas del final (se leen de derecha a izquierda)
            prov_text = clean_row[-1] if num_cols >= 10 else ""
            epp_raw = clean_row[-2] if num_cols >= 10 else clean_row[-1]
            un_code_raw = clean_row[-3] if num_cols >= 10 else clean_row[-2]
            un_class_raw = clean_row[-4] if num_cols >= 10 else clean_row[-3]
            
            middle_cols = clean_row[1 : -4] if num_cols >= 10 else clean_row[1 : -3]
            
            # Detectar la Palabra de Advertencia en middle_cols
            sig_idx = -1
            for m_i, item in enumerate(middle_cols):
                item_low = item.lower().strip()
                if any(w in item_low for w in ["atencion", "atención", "peligro", "nonhazardous", "sin palabra"]):
                    sig_idx = m_i
                    break
            
            if sig_idx != -1:
                comp_parts = middle_cols[:sig_idx]
                c_text = " ".join([c for c in comp_parts if c]).replace("|", "\n")
                signal_raw = middle_cols[sig_idx]
                sga_raw = middle_cols[sig_idx + 1] if sig_idx + 1 < len(middle_cols) else ""
                rem_cols = middle_cols[sig_idx + 2 :]
            else:
                c_text = middle_cols[0] if middle_cols else ""
                signal_raw = middle_cols[1] if len(middle_cols) > 1 else "Atención"
                sga_raw = middle_cols[2] if len(middle_cols) > 2 else ""
                rem_cols = middle_cols[3:]
                
            # Separar rem_cols en Frases H y Frases P según patrones GHS
            p_start_idx = -1
            for r_i, r_item in enumerate(rem_cols):
                if re.search(r'^\s*P\d{3}', r_item, re.IGNORECASE) or re.search(r'^\s*(P\d{3}|sin consejo|ningun consejo|no requiere)', r_item, re.IGNORECASE):
                    p_start_idx = r_i
                    break
            
            if p_start_idx != -1:
                h_parts = rem_cols[:p_start_idx]
                p_parts = rem_cols[p_start_idx:]
            else:
                if rem_cols:
                    h_parts = [rem_cols[0]]
                    p_parts = rem_cols[1:]
            ...
            h_raw = " ".join([h for h in h_parts if h])
            p_raw = " ".join([p for p in p_parts if p])
