...
# --- CRÉDITOS Y COPYRIGHT EN BARRA LATERAL ---
st.sidebar.divider()
st.sidebar.markdown(
    """
    <div style="text-align: center; color: #4B5563; font-size: 0.85rem; padding: 10px; background-color: #F3F4F6; border-radius: 8px;">
        ...
        <strong>SUSAN CABRERA ENRIQUEZ/ UNIVERSIDAD DE SAN BUENAVENTURA</strong><br>
...
def parse_chemical_db(content_str, sga_files, epp_files, un_files):
    """
    Parsea de forma estándar y limpia la base de datos de reactivos (CSV/TSV/delimitado por punto y coma ;).
    Lee las columnas por su posición estándar (0: Nombre, 1: Composición, 2: Palabra, 3: SGA, 4: Frases H, 
    5: Frases P, 6: Clase UN, 7: Código UN, 8: EPP, 9: Proveedor) o mediante mapeo de encabezados.
    """
    ...
    first_row = [str(cell).strip() for cell in all_rows[0]]
    first_row_str = " ".join(first_row).lower().replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u')

    header_keywords = ["nombre", "sustancia", "reactivo", "producto", "composicion", "palabra", "advertencia"]
    is_header = any(k in first_row_str for k in header_keywords) and not ("cas" in first_row_str or "ghs0" in first_row_str or "h3" in first_row_str)
    ...
    if is_header:
        ...
        for idx, h in enumerate(headers):
            clean_h = str(h).strip().lower().replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u')
    ...
    for row in data_rows:
        ...
        # Si la fila es un encabezado repetido, omitir
        if clean_row[0].upper() in ["NOMBRE", "SUSTANCIA", "REACTIVO", "NOMBRE_DEL_PRODUCTO", "PRODUCTO"]:
            continue

        p_name = clean_row[hdr_map.get("nombre", 0)] if "nombre" in hdr_map and hdr_map["nombre"] < len(clean_row) else clean_row[0]
        
        if not p_name or p_name.upper() in ["NOMBRE", "SUSTANCIA", "REACTIVO", "NOMBRE_DEL_PRODUCTO"]:
            continue

        c_text = clean_row[hdr_map.get("composicion", 1)].replace("|", "\n") if ("composicion" in hdr_map and hdr_map["composicion"] < len(clean_row)) or len(clean_row) > 1 else (clean_row[1].replace("|", "\n") if len(clean_row) > 1 else "")
        signal_raw = clean_row[hdr_map.get("palabra", 2)] if ("palabra" in hdr_map and hdr_map["palabra"] < len(clean_row)) else (clean_row[2] if len(clean_row) > 2 else "Atención")
        sga_raw = clean_row[hdr_map.get("sga", 3)] if ("sga" in hdr_map and hdr_map["sga"] < len(clean_row)) else (clean_row[3] if len(clean_row) > 3 else "")
        h_raw = clean_row[hdr_map.get("frase_h", 4)] if ("frase_h" in hdr_map and hdr_map["frase_h"] < len(clean_row)) else (clean_row[4] if len(clean_row) > 4 else "")
        p_raw = clean_row[hdr_map.get("frase_p", 5)] if ("frase_p" in hdr_map and hdr_map["frase_p"] < len(clean_row)) else (clean_row[5] if len(clean_row) > 5 else "")
        un_class_raw = clean_row[hdr_map.get("clase_un", 6)] if ("clase_un" in hdr_map and hdr_map["clase_un"] < len(clean_row)) else (clean_row[6] if len(clean_row) > 6 else "")
        un_code_raw = clean_row[hdr_map.get("codigo_un", 7)] if ("codigo_un" in hdr_map and hdr_map["codigo_un"] < len(clean_row)) else (clean_row[7] if len(clean_row) > 7 else "")
        epp_raw = clean_row[hdr_map.get("epp", 8)] if ("epp" in hdr_map and hdr_map["epp"] < len(clean_row)) else (clean_row[8] if len(clean_row) > 8 else "")
        prov_text = clean_row[hdr_map.get("proveedor", 9)] if ("proveedor" in hdr_map and hdr_map["proveedor"] < len(clean_row)) else (clean_row[9] if len(clean_row) > 9 else "")
...
def generate_chemical_label_custom(
    ...
    font_body         = load_font(19 * scale, is_bold=True)   # Frases H y Frases P (Negrita para imprenta)
    font_body_bold    = load_font(17 * scale, is_bold=True)  # Encabezados de sección
    font_small        = load_font(15 * scale, is_bold=True)   # Texto de Composición (Negrita para imprenta)
    ...
    # Solo reduce la fuente si el texto genera más de 3 líneas o supera 110px de altura
    while font_size_comp > min_font_size_comp and (len(comp_lines) > 3 or comp_h > (110 * scale)):
        font_size_comp -= 1
        font_comp = load_font(font_size_comp, is_bold=True)
        comp_lines, comp_h, _ = wrap_and_measure_text(draw, c_text, font_comp, comp_max_w)
    
    # Ajuste dinámico de tamaño de fuente para el Nombre del Producto:
    # Si el título es de una sola palabra (ej: ACETILSALICILICO, DICLOROMETANO), la letra se reduce
    # progresivamente hasta que la palabra quepa completa en 1 sola línea sin cortarse ni dividirse.
    max_prod_w = mid_col_w - (PADDING_INNER * 2)
    font_size_prod = int(38 * scale)
    min_font_size_prod = int(8 * scale)  # Permite reducir hasta 8pt para palabras individuales muy largas
    ...
    prod_words = p_name.upper().strip().split()
    
    def title_needs_reduction(f_size, f_font, lines, h):
        if f_size <= min_font_size_prod:
            return False
        # Si el título es de una sola palabra, no debe dividirse ni separarse en múltiples líneas
        if len(prod_words) <= 1:
            if len(lines) > 1:
                return True
            w_bbox = draw.textbbox((0, 0), p_name.upper(), font=f_font)
            if (w_bbox[2] - w_bbox[0]) > max_prod_w:
                return True
            return False
        else:
            # Si tiene varias palabras, ninguna palabra individual debe ser dividida
            for w in prod_words:
                wb = draw.textbbox((0, 0), w, font=f_font)
                if (wb[2] - wb[0]) > max_prod_w:
                    return True
            if len(lines) > 3 or h > 85:
                return True
            return False

    while title_needs_reduction(font_size_prod, font_prod_name, prod_lines, prod_h):
        font_size_prod -= 1
        font_prod_name = load_font(font_size_prod, is_bold=True)
        prod_lines, prod_h, _ = wrap_and_measure_text(draw, p_name.upper(), font_prod_name, max_prod_w)
    ...
    comp_box_h = row1_h - 30
    cy = y_curr + 30 + max(4, (comp_box_h - comp_h) // 2)
    for line in comp_lines:
        line_bbox = draw.textbbox((0, 0), line, font=font_comp)
        lw = line_bbox[2] - line_bbox[0]
        lx = max(comp_x_start + PADDING_INNER, comp_x_start + (col_left_w - lw) // 2)
        draw.text((lx, cy), line, fill="black", font=font_comp)
        cy += font_comp.size + 4
