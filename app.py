...
def parse_chemical_db(content_str, sga_files, epp_files, un_files):
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        # Limpiar espacios y celdas vacías del final de la fila
        ...
        def get_val(key, default_pos):
            if key in hdr_map and hdr_map[key] < num_cols:
                return clean_row[hdr_map[key]]
            elif default_pos is not None and abs(default_pos) < num_cols:
                return clean_row[default_pos]
            return ""

        # Si la fila tiene 10 o más columnas, los campos fijos están al final (de derecha a izquierda)
        if num_cols >= 10:
            p_name = clean_row[0]
            prov_text = clean_row[-1]
            epp_raw = clean_row[-2]
            un_code_raw = clean_row[-3]
            un_class_raw = clean_row[-4]
            p_raw = clean_row[-5]
            h_raw = clean_row[-6]
            sga_raw = clean_row[-7]
            signal_raw = clean_row[-8]
            comp_parts = [c for c in clean_row[1:-8] if c]
